/* OMEN app.js — mount after DOM is ready */
(function(){
  "use strict";
  ...
  ready(function init(){
    ...
    /* ------- Make anchors non-clickable on desktop ------- */
    if (!isTouch) {
    ...
    function armHudHide(){
      ...
      hudHideTimer = setTimeout(function(){ if (!overHud) closeHud(); }, 180);
    }
    ...
    /* Mobile: tap to stick, tap outside to close */
    if (isTouch){
    ...
    function oracleOpen(){
      var idx = getDailyIndex(MOMENTS.length);
      showOracle(MOMENTS[idx] || {});
    }
    ...
    function resetLayer(layer){
      ...
      void layer.offsetWidth; // reflow flush
    ...
    function startLayer(layer){
      ...
      // Fade timings (2s fades) relative to 16s loop
      ...
      timers.push(setTimeout(function(){ var s=layer.querySelector(".top");    if(s) s.classList.add("is-fading"); }, 16000)); // 16s → 18s (hidden as layer B begins)
    ...
    function barStart(thick){
      ...
      startLayer(layerA);                             // start now
      startBto = setTimeout(function(){               // stagger B by 16s so it begins as A starts to fade
        startLayer(layerB);
      }, 16000);

      // Repeat every 32s with the same staggering
    ...
    function clearWeeklyLabelTimers(){
      for (var i=0;i<labelTs.length;i++) clearTimeout(labelTs[i]);
      labelTs = [];
      if (labelLoop){ clearInterval(labelLoop); labelLoop = null; }
    }
    function runWeeklyLabelCycleOnce(){
      var seq = ["INHALE","HOLD","EXHALE","HOLD"];
      for (var i=0;i<seq.length;i++){
        (function(word,delay){
          labelTs.push(setTimeout(function(){
            if (weeklyOpen && breatheLink) breatheLink.textContent = word;
          }, delay));
        })(seq[i], i*4000);
      }
    }
    function restartWeeklyLabels(){
      clearWeeklyLabelTimers();
      runWeeklyLabelCycleOnce();
      labelLoop = setInterval(function(){ clearWeeklyLabelTimers(); runWeeklyLabelCycleOnce(); }, 16000);
    }
    ...
    function showWeekly(){
      ...
      weeklyOpen = true;
      barStart(false);          // thin bars
      restartWeeklyLabels();    // swap BREATHE label while hovering weekly
    }
    ...
    function hideWeekly(){
      ...
      weeklyOpen = false;
      clearWeeklyLabelTimers();
      if (breatheLink) breatheLink.textContent = "BREATHE";
      barStop();
    }
    ...
    function enterBreathe(){
      hideWeekly();
      if (breatheOverlay){ breatheOverlay.classList.add("is-open"); breatheOverlay.setAttribute("aria-hidden","false"); }
      barStart(true);   // thick bars
      startCenterWords();
    }
    function exitBreathe(){
      stopCenterWords();
      barStop();
      if (breatheOverlay){ breatheOverlay.classList.remove("is-open"); breatheOverlay.setAttribute("aria-hidden","true"); }
    }
    ...
    /* Optionally close HUD/Breathe via ESC */
