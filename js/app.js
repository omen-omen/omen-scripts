/* OMEN app.js (self-mounting, snake timing fixed) */
(function () {
  "use strict";

  function ready(fn){ if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",fn);} else {fn();} }

  /* -------- Inject scaffold if missing (Carrd-friendly) -------- */
  ...
  ready(function init(){
    ...
    /* ------- Make anchors non-clickable on desktop (hover-only UX) ------- */
    if(!isTouch){
    ...
    /* ================= Snake (double buffer, unified fade @16s) ================= */
    ...
    function resetLayer(layer){
      ...
      // reflow to restart keyframes cleanly
    ...
    function startLayer(layer){
      ...
      var T = 16000; // all four sides begin 2s fade together at 16s
    ...
    function barStart(thick){
      ...
      // Start Layer A now; Layer B starts at 16s for seamless overlap
      startLayer(layerA);
    ...
    function weeklyText(){
      ...
      if (Array.isArray(src)) list = src;
        ...
        for (var k in src) if (Object.prototype.hasOwnProperty.call(src,k) && Array.isArray(src[k])) list = list.concat(src[k]);
      }
